...
public class ListPanel extends ListPanelType implements MouseInputListener, MouseWheelListener {
    ...
    protected void paintComponent(Graphics g) {
        ...
        ((OrderedList) list).draw(g2, 0, new Point(Settings.indent, Settings.indent),
                ((OrderedList) list).hover(g2, lastpos.x, lastpos.y - (int) scroll), false);
        g2.translate(0, -scroll);
    }
    ...
    public void mousePressed(MouseEvent e) {
        ...
        if (clicked != null)
            selectdrag = clicked.equals(((OrderedList) list).getfocus());
    }

    public void mouseReleased(MouseEvent e) {
        if (Math.abs(clickpos.x - e.getX()) < 5 && Math.abs(clickpos.y - e.getY()) < 5) {
            OrderedList clicked = ((OrderedList) list).get(g2, e.getX() - Settings.indent,
                    e.getY() - Settings.indent - (int) scroll);
            if (clicked != null)
                clicked.click(g2, e.getX());
        } else if (!selectdrag) {
    ...
    public void mouseDragged(MouseEvent e) {
        if (selectdrag) {
            ...
            if (!(corrFoc == null || corrFoc.holder == null || corrMoused == null || corrMoused.holder == null)) {// do
                                                                                                                  // nothing
                if (corrFoc.equals(corrMoused)) {
                    if (e.getX() - clickpos.getX() > Settings.indent) {
                        ...
                        if (corrFoc.holder.holder != null
                                && foc.holder.items.indexOf(foc) == foc.holder.items.size() - 1) {
                            clickpos.x -= Settings.indent;
                            ((OrderedList) foc.holder).remove(foc);
            ...
            if (scroll > 0)
                scroll = 0;
            var end = -Settings.line + getHeight()
                    + (-list.countit()) * (1.0 * Settings.line + list.correct().name.lineheight(g2));
            if (scroll < end)
                scroll = end;
        }
        lastpos = e.getPoint();
            ...
            if (e.isControlDown()) {
                ...
                if (hold == null) {
                    ...
                    try {
                        ...
                        if (focuus.cursor == foc.name.getValue().length())
                            foc.name.setValue(foc.name.getValue() + data);
                        else
                            foc.name.setValue(foc.name.getValue().substring(0, focuus.cursor) + data
                                    + foc.name.getValue().substring(focuus.cursor, foc.name.getValue().length()));
                        int endx = (1 + foc.level()) * Settings.indent + foc.name.linewidth(g2) + Settings.linespace
                                + 5 * foc.name.lineheight(g2);
                ...
                if (foc.holder != null)
                    focuus.setfocus((OrderedList) ((List) foc.prev()).instance(list));
                ...
                foc.name.setValue(foc.name.getValue().substring(0, focuus.cursor - 1)
                        + foc.name.getValue().substring(focuus.cursor, foc.name.getValue().length()));
                focuus.cursor--;
                ...
                if (focuus.cursor == foc.name.getValue().length())
                    ...
                    foc.name.setValue(foc.name.getValue().substring(0, focuus.cursor) + e.getKeyChar()
                            + foc.name.getValue().substring(focuus.cursor, foc.name.getValue().length()));
                int a = (1 + foc.level()) * Settings.indent + foc.name.linewidth(g2) + Settings.linespace
                        + 5 * foc.name.lineheight(g2);
