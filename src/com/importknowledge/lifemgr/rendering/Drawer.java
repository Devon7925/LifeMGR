...
public class Drawer {
    ...
    public void drawEllipCent(Rectangle2D r) {
        g2.fillOval(transx(r.getX()) - transw(r.getWidth()) / 2, transy(r.getY()) - transh(r.getHeight()) / 2,
                transw(r.getWidth()), transh(r.getHeight()));
    }
    ...
    public void drawLine(double x, double y, double x1, double y1, Color c) {
        g2.setStroke(new BasicStroke(5));
        g2.drawLine(transx(x), transy(y), transx(x) + (transx(x1) - transx(x)) / 2,
                transy(y) + (transy(y1) - transy(y)) / 2);
        g2.setColor(c);
        g2.drawLine(transx(x) + (transx(x1) - transx(x)) / 2, transy(y) + (transy(y1) - transy(y)) / 2, transx(x1),
                transy(y1));
        setColor(colour);
    }
